...
### US-2.0b 복습 중 "알아요/몰라요" 자기평가 (신규, 2026-09-14)
- [x] 카드를 뒤집으면 "몰라요"/"알아요" 버튼이 나오고, 누르면 `vocab_word_marks`에 저장되어 이후 세션에서도 "예전에 알아요로 표시함" 배지로 보인다. 복습이 끝나면 "헷갈리는 단어(몰라요)만 다시 복습"으로 좁혀서 반복할 수 있다.
- 근거: `vocab_word_marks` 테이블(0006) / `ReviewSession.tsx`, `/api/vocab-word-marks`
