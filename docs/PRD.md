...
### 4.11 (2026-09-15) 복습 모드 발음 음소거 + 부모용 학습 현황 대시보드

두 가지를 사용자 요청으로 추가:
- **발음 음소거**: 복습 모드에서 "필요에 따라 음소거하고 학습할 수 있게" — 🔊/🔇 토글 버튼(카운터 옆), 상태는 기기별 `localStorage`에 저장(서버 데이터 아님). `speakEnglish()` 자체가 호출 시점에 음소거 여부를 확인하므로, 앞으로 시험 도전 화면에도 발음을 붙이면 자동으로 음소거가 적용된다.
- **부모용 학습 현황 대시보드**: "부모가 학습 현황을 볼 수 있는 대시보드" — `/dashboard`(자녀별 카드: 단어/DAY 개수·정답률·⭐ 누적·✅/🤔·최근 활동)와 `/dashboard/[childId]`(DAY별 진행 상황 표 + 최근 학습 기록). `/profiles`에 진입 링크 추가. 쌍둥이 간 순위/비교 연출은 의도적으로 넣지 않았다 — 9장에서 랭킹은 Phase 3으로 보류하기로 한 결정과 일관되게, 카드를 나열만 하고 서로 비교하는 숫자는 만들지 않았다. 상세는 [ARCHITECTURE.md](./ARCHITECTURE.md) 12장.

### 4.12 (2026-09-15) 회원가입 없이 체험하는 DAY 1 데모

"로그인 첫 화면에 DAY 1을 회원가입 없이 경험할 수 있는 데모"를 요청 — `/login`에 진입 버튼을 추가해 `/demo`(허브: 복습 + 시험 도전 3종) → `/demo/{review,typing,choice,arrange}`로 연결한다.

- 실제 능률보카 중등기본 DAY 01 CSV에서 뜻이 깨끗한 단어 10개만 골라 하드코딩(`src/lib/demoWords.ts`) — 실 DB 데이터와는 완전히 분리(id가 `demo-` 접두사).
- 로그인 세션도 실제 단어 id도 없어서 서버 채점 API를 쓸 수 없으므로, 채점은 전부 클라이언트에서 하고 아무것도 저장하지 않는다(1회성 체험). 디스트랙터·글자 타일 생성 로직은 실 서비스와 100% 동일한 함수를 재사용해서 "가짜 체험"이 아니게 만들었다.
- 상세 설계와, 테스트 중 발견해 함께 고친 하이드레이션 버그는 [ARCHITECTURE.md](./ARCHITECTURE.md) 13장·14장 참고.
...
- **Phase 1** — 점검 (MVP, 등록은 CSV 가져오기로 대체 완료)
  ...
  - [x] 4.6. (추가) 복습 모드 발음 음소거 토글 — 4.11 참고
  - [x] 4.7. (추가) 부모용 학습 현황 대시보드(`/dashboard`) — 4.11 참고
  - [x] 4.8. (추가) 회원가입 없이 체험하는 DAY 1 데모(`/demo`) — 4.12 참고
