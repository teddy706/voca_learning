...
- [x] 11. (추가 항목) 단어장 목록에 학습 이력 요약(⭐/✅/🤔) 표기 — 2026-09-14 사용자 요청, 아래 참고
...
### 모든 화면에 뒤로가기 버튼 (2026-09-14)
사용자가 "모든 페이지에서 뒤로가기(취소) 버튼이 있으면 좋겠다"고 요청 — 특히 시험 도전/복습 중간에 나갈 방법이 없던 게 문제였다.
- `src/components/BackLink.tsx`(리딩버디 그대로 포팅, `.btn-pill` 클래스도 함께 추가) — 화면 맨 위에 "← 뒤로" pill 버튼.
...
- **(2026-09-14 추가) 복습을 끝까지 안 해도 모드 허브(`/check/[batchId]`)에서 바로 보임**: 처음엔 "헷갈리는 단어만 다시 복습"이 복습 세션이 끝난 뒤 결과 화면에만 있어서, 사용자가 "지금은 복습이 다 끝나고 난 후만 보인다"고 지적했다. `/check/[batchId]` 모드 허브가 이제 `getWordMarks`로 `unknown` 표시된 단어를 직접 조회해서 "🤔 헷갈리는 단어 (N개)" 카드로 목록을 바로 보여주고, "이 단어만 다시 복습" 버튼이 `/check/[batchId]/review?only=unknown`으로 연결된다 — 복습 페이지는 이 쿼리 파라미터를 보고 서버에서 미리 걸러서 세션을 시작한다.
